...
func InitFlags(p *config.PrometheusConfig) {
	...
	flags.StringEnvVar(&p.StorageClassProvisioner, "prometheus-storage-class-provisioner", "PROMETHEUS_STORAGE_CLASS_PROVISIONER", "docker.io/hostpath", "Volumes plugin used to provision PVs for Prometheus.")
	...
	flags.StringEnvVar(&p.PVCStorageClass, "prometheus-pvc-storage-class", "PROMETHEUS_PVC_STORAGE_CLASS", "hostpath", "Storage class used with prometheus persistent volume claim.")
	...
	flags.StringEnvVar(&p.PrometheusMemoryRequest, "prometheus-memory-request", "PROMETHEUS_MEMORY_REQUEST", "1Gi", "Memory request to be used by promehteus.")
